...
char** f_first_array(std::string filename) {
...
// Функция обновляет пространство
char** f_recursion_arr(char** arr_live, int fv_row, int fv_col) {
    while (true) {
        ...
        for (int ic = 0; ic < fv_row; ic++) {
            for (int jc = 0; jc < fv_col; jc++) {
                arr_live[ic][jc] = arrf_liven[ic][jc];
            }
        }
        f_delete_array(arrf_liven, fv_row, fv_col);
        int vf_live_all = f_live_all(arr_live, fv_row, fv_col);
        gv_itaration++;
        f_print_array(arr_live, fv_row, fv_col, vf_live_all);
        Sleep(100);
        if (vf_status == 0) {                                                           // Если в мире ни чего не меняется то программа останавливается
            ...
            f_delete_array(arr_live, fv_row, fv_col);
            return EXIT_SUCCESS;
        }
        ...
        else if (gv_itaration == 100) {                                                 // Если количество итераций превышает 200 - программа останавливается
...
int main()
{
    ...
    std::cout << "Приветствую вас в игре «Жизнь», " << std::endl;
    std::cout << "придуманная английским математиком Джоном Конвеем в 1970 году." << std::endl;
    std::cout << "Сформируйте файл .txt согласно правилам и положите в папку с игрой." << std::endl;
    std::cout << "Введите название файла: ";
    std::cin >> vm_filename;
    std::cout << "Спасибо! Вы все правильно сделали. Начинаем..." << std::endl;
    Sleep(1000);
    char** m_arr = f_first_array(vm_filename);
